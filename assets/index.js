function main() {
    ...
    const checkboxRank = document.getElementsByClassName('header_btn-rank')[0];
    ...
    function handle(data) {
        ...
        if (checkboxRank) {
            const optionExists = Array.from(checkboxRank.options).some(opt => opt.value === savedSettings.rank);
            checkboxRank.value = optionExists ? savedSettings.rank : '0';
        }
        if (checkboxRandom) {
            checkboxRandom.checked = savedSettings.random;
        }
        ...
        document.addEventListener('keydown', (e) => {
            if (e.key === 'Escape' || e.code === 'ArrowUp' || e.code === 'ArrowDown' || e.code === 'KeyF') {
                toggleDisplay();
            }
        ...
        checkboxRank.addEventListener('change', () => {
            saveSettings();
            applyFilter();
            sortFiltered();
            displayCurrent();
        ...
        function applyFilter() {
            const selectedRank = checkboxRank.value;
        ...
        function saveSettings() {
            const settings = {
                rank: checkboxRank ? checkboxRank.value : '0',
                random: checkboxRandom ? checkboxRandom.checked : false
            };
        ...
        function loadSettings() {
            try {
                ...
                if (!stored) return { rank: '0', random: false };
                const parsed = JSON.parse(stored);
                return {
                    rank: typeof parsed.rank === 'string' ? parsed.rank : '0',
                    random: !!parsed.random
                };
            } catch (err) {
                console.warn('Failed to load saved settings:', err);
                return { rank: '0', random: false };
            }
        }
        ...
        function createRankFilterE(dataMapped) {
            ...
            appearingRanks.forEach(rank => {
                ...
                checkboxRank.appendChild(option);
            });
